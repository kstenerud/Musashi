.include "entry.s"
/* OPCODE : BFSET*/
/*-----------------------------------------------------------*/
/*-----------------------------------------------------------*/

.set SRC_LOC,   STACK2_BASE - 0x100
.set BF_LOC1, SRC_LOC+0
.set BF_LOC2, SRC_LOC+4


/** See UM page 40*/

op_BFSET:
    mov.l #0x01234567, BF_LOC1
    bfset BF_LOC1, #12, #8 | Change 0x34 => 0xFF
    beq TEST_FAIL | Z = 0
    bmi TEST_FAIL | N = 0
    cmp.l #0x12FF567, BF_LOC1
    bne TEST_FAIL

    /* Same thing, with data register */
    mov.l #0x01234567, BF_LOC1
    mov.l #12, %d0
    bfset BF_LOC1, %d0, #8 | Change 0x34 => 0xFF
    beq TEST_FAIL | Z = 0
    bmi TEST_FAIL | N = 0
    cmp.l #0x12FF567, BF_LOC1
    bne TEST_FAIL

    /* Data register has "wrap around" behavior */
    mov.l #0x01234565, %d1
    mov.l #30, %d0
    bfset %d1, %d0, #4 | Change 0x65 => 0x67, 01 => C1
    beq TEST_FAIL | Z = 0
    bmi TEST_FAIL | N = 0
    cmp.l #0xC1234567, %d1
    bne TEST_FAIL

    /* Same thing, but offseted into oblivion*/
    mov.l #0x01234567, BF_LOC1
    mov.l #12+BF_LOC1*8, %d0
    bfset 0, %d0, #8 | Change 0x34 => 0xFF
    beq TEST_FAIL | Z = 0
    bmi TEST_FAIL | N = 0
    cmp.l #0x12FF567, BF_LOC1
    bne TEST_FAIL

    /* Same thing, but negative offset */
    mov.l #0x01234567, BF_LOC1
    mov.l #12+(BF_LOC1-0xABCDEF)*8, %d0
    bfset 0xABCDEF, %d0, #8 | Change 0x34 => 0xFF
    beq TEST_FAIL | Z = 0
    bmi TEST_FAIL | N = 0
    cmp.l #0x12FF567, BF_LOC1
    bne TEST_FAIL

    /** Touch 5 bytes **/
    mov.l #0x01234566, BF_LOC1
    mov.l #0x89ABCDEE, BF_LOC2
    bfset BF_LOC1, #31, #32 | Change 0x66 => 0x67, 0x89ABCDEE => 0xFFFFFFFE
    beq TEST_FAIL | Z = 0
    bmi TEST_FAIL | N = 0
    cmp.l #0x1234567, BF_LOC1
    bne TEST_FAIL
    cmp.l #0xFFFFFFFE, BF_LOC2
    bne TEST_FAIL

    /** Zero flag **/
    mov.l #0x00, BF_LOC1
    mov.l #0x00, BF_LOC2
    bfset BF_LOC1, #31, #32
    bne TEST_FAIL | Z = 1
    bmi TEST_FAIL | N = 0
    cmp.l #0x1, BF_LOC1
    bne TEST_FAIL
    cmp.l #0xFFFFFFFE, BF_LOC2
    bne TEST_FAIL


    rts


