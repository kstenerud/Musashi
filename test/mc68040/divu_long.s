...
op_DIVU_L:
    ...
    mov.l #0x12345, %d0
    mov.l #1, %d1 | Makes sure that we actually execute divul.l and not divu.l
    divul.l #0x7, %d1, %d0
    beq TEST_FAIL
    bmi TEST_FAIL
    bvs TEST_FAIL
...
divull_inner:
    ...
    mov.l %d4, %d5
    mov.w #0, %ccr
    divul.l %d3, %d0, %d5 | Div %d5 / %d3, result in %d5, rem in %d0
    bvs TEST_FAIL
...
divul_inner:
    ...
    mov.l #0, %d7
    mov.w #0, %ccr
    divu.l %d3, %d0, %d6 | Div %d0-%d6 / %d3, result in %d6, rem in %d0
    mov.w %ccr, %d7
